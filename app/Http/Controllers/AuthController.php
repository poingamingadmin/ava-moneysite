...
class AuthController extends Controller
{
    ...
    public function deposits_store(Request $request)
    {
        ...
        if ($transaction) {
            $message = "There is a new deposit from {$user->username} amounting to $ {$request->deposite_amount}";
            $this->pusher_notification('deposit', $message);
            $this->pusher_deposit($transaction, $user);
            $message = "Ticket deposit Anda sebesar ${$request->deposite_amount} telah berhasil dan telah masuk ke antrian untuk diproses. Mohon menunggu sebentar sampai proses deposit selesai.";
    ...
    public function withdrawals_store(Request $request)
    {
        ...
        $response = ApiTransactionService::withdraw($user->player_token, $amount);
        ...
        if ($transaction) {
            ...
            $message = "Ticket withdrawal Anda sebesar $ {$request->withdrawal_amount} telah berhasil dan telah masuk ke antrian untuk diproses. Mohon menunggu sebentar sampai proses withdrawal selesai.";
    ...
    public function getBal()
    {
        try {
            ...
            if ($user->is_playing) {
                $response = ApiTransactionService::getBalance($user->player_token);

                $status = $response['data']['status'] ?? null;
                $balance = $response['data']['user_balance'] ?? null;
            ...
            return [
                'data' => number_format($user->active_balance, 2)
            ];
        } catch (\Throwable $e) {
            ...
            return [
                'data' => number_format(Auth::user()->active_balance, 2)
            ];
        }
    }
}
