...
class ViewController extends Controller
{
    ...
    {
        $cacheKey    = 'firebase_data_' . strtolower($provider) . '_' . $category;
        $firebaseUrl = env('FIREBASE_HOST') . "/provider.json";
        ...
        if (empty($data)) {
            try {
                ...
                if ($response->successful() && ! empty($response->json())) {
                    $data = $response->json();
                    Cache::put($cacheKey, $data, now()->addMonth()); // Simpan cache 1 bulan
                } else {
                    Log::warning("Data dari Firebase ({$provider}) kosong atau tidak valid.");
                    $data = [];
                }
            } catch (\Throwable $e) {
                Log::error("Gagal mengambil data Firebase ({$provider}): " . $e->getMessage());
                $data = [];
            }
        }
        ...
        return view(user_agent() . '.show-games', [
            'data' => [
                'category'      => $category,
                'provider'      => $data[$category][$provider],
                'provider-code' => $data[$category][$provider][0]['provider'] ?? null,
            ],
        ]);
    }
